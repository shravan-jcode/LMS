...
The project uses environment variables for sensitive configuration such as database credentials, API keys, and payment credentials.
...
You can use `server/.env.example` as a reference.
...
# Frontend URL
VITE_FRONTEND_URLL=http://localhost:5173
```
...
Use `client/.env.example` as a reference and add the required frontend environment variables.

> **Never commit `.env` files, API keys, passwords, database credentials, or other sensitive information to GitHub.**
...
The exact commands and ports may vary depending on your project's `package.json` and configuration.
