...
const CourseProgress = () => {
  ...
  if (isLoading) return <p className="text-center mt-10 text-gray-800 dark:text-gray-200">Loading course details...</p>;
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-gray-100">{courseTitle}</h1>
        <Button
          onClick={completed ? handleInCompleteCourse : handleCompleteCourse}
          variant={completed ? "outline" : "default"}
          className={`mt-2 md:mt-0 rounded-full px-5 ${
            completed 
                ...
                <CardContent className="flex justify-between items-center p-4">
                  <div className="flex items-center gap-3">
                    {isLectureCompleted(lecture._id) ? (
                      ...
                      <CirclePlay size={24} className="text-gray-500 dark:text-gray-400" />
                    )}
                    <CardTitle className="text-md md:text-lg font-medium text-gray-800 dark:text-gray-200">{lecture.lectureTitle}</CardTitle>
                  </div>
                  {isLectureCompleted(lecture._id) && (
                    <Badge className="bg-green-500/20 text-green-700 dark:bg-green-500/10 dark:text-green-400">Completed</Badge>
                  )}
                </CardContent>
              </Card>
            ))}
